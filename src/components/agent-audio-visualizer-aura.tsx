...
import { preferMobileShader } from '@/lib/device/is-ios';
...
function createAuraShaderSource(iterations: number): string {
  return `
...
const shaderSourceDesktop = createAuraShaderSource(36);
const shaderSourceMobile = createAuraShaderSource(12);

function AuraCssFallback() {
  return (
    <div
...
function AuraShader({
  ...
  const [useMobileShader, setUseMobileShader] = useState(false);

  useEffect(() => {
    setUseMobileShader(preferMobileShader());
  }, []);

  const shaderSource = useMobileShader ? shaderSourceMobile : shaderSourceDesktop;
  const shaderPrecision = useMobileShader ? 'mediump' : 'highp';

  return (
    <div ref={ref} className={cn('relative', className)} {...props}>
      {webglFailed ? <AuraCssFallback /> : null}
      <ReactShaderToy
        fs={shaderSource}
        ...
        style={{ width: '100%', height: '100%', visibility: webglFailed ? 'hidden' : 'visible' }}
      />
    </div>
  );
}
